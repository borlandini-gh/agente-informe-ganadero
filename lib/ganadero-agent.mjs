...
function buildCanvaFields(data) {
  ...
  for (let monthIndex = 7; monthIndex < 12; monthIndex += 1) {
    ...
    fields[`${tag}_total`] = fields[`${sourcePrefix}_total`] || ZERO_WIDTH_SPACE;
    fields[`${tag}_certificados`] = fields[`${sourcePrefix}_certificados`] || ZERO_WIDTH_SPACE;
    fields[`${tag}_valorcp`] = fields[`${sourcePrefix}_valor_cp`] || ZERO_WIDTH_SPACE;
    fields[`${tag}_var`] = fields[`${sourcePrefix}_variacion`] || ZERO_WIDTH_SPACE;
  }
  return fields;
}
